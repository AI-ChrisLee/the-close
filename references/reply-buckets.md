...
| Bucket | What it sounds like | The move | The next touch |
|---|---|---|---|
| Interested | "send the link", "let's talk", "when can you start" | One next step, this hour | Link sent: day 4, one nudge if the money has not cleared, then stop. Call booked: the call itself, nothing else scheduled |
| Question | "how much", "do you do X", "how long" | Answer from the offer document, ask one back. A priced question whose scope fits one sentence is interested instead, and gets the way to pay | Day 4, one bump, then the not-now shelf |
| Objection | "too expensive", "we have someone", "we'd just use AI" | Test it, never answer it | Day 4, one bump; if they named the real blocker, the date it clears |
| Not now | "after the summer", "we're mid-launch" | Get the date in their words | The date they gave, quoted; no date given, ask for one |
| No | "not interested", "remove me", anything hostile | One line, the shortest in the batch, row closed | None, ever |

A reply with no ask in it gets no bucket and one real question back. The bucket's next touch is
the date that goes in the pipeline row's next-touch field; a date nobody wrote down is a
follow-up that never fires.
...
- A handoff inside their own company (talk to my partner, email our ops lead) is **interested**,
  ...
  introduction. A referred name is the strongest lead on any list.
...
- **An auto-reply is not a reply.** An out of office carries no ask, so it gets no bucket and no
  draft, and it never counts in the day's replies number. It still moves that person's next
  touch in the pipeline row to the day after the return date they gave, quoted in their words.
  A return date nobody wrote down is a person who gets messaged while they are away.

## The door, for the interested ones only

Printed as one word plus a one-line reason so the founder can flip it. The **link** when the offer
document carries the price and the scope fits one sentence; the **call** when either one is fuzzy. The
link message is the payment path, that one sentence of scope, a start date, and an invitation to
raise anything before paying. The call message carries the booking link and
